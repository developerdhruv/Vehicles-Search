...
function SearchPage() {
    ...
    useEffect(() => {
        const fetchCategories = async () => {
            try {
                const response = await axios.get('http://65.0.122.72:3000/api/categories');
    ...
    useEffect(() => {
        const fetchSuggestions = async () => {
            if (keyword.length > 1) {
                try {
                    const response = await axios.get('http://65.0.122.72:3000/api/suggestions', {
                        params: { term: keyword }
                    });
    ...
    useEffect(() => {
        const fetchMakeSuggestions = async () => {
            if (make.length > 1) {
                try {
                    const response = await axios.get('http://65.0.122.72:3000/api/makes', {
                        params: { term: make }
                    });
    ...
    useEffect(() => {
        const fetchModelSuggestions = async () => {
            if (model.length > 1) {
                try {
                    const response = await axios.get('http://65.0.122.72:3000/api/models', {
                        params: { term: model }
                    });
    ...
    const handleSearch = async () => {
        ...
        setError('');
        try {
            const response = await axios.get('http://65.0.122.72:3000/api/products', {
